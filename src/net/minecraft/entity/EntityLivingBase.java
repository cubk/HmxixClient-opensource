    ...
    private int getArmSwingAnimationEnd()
    {
        if (this == Minecraft.getMinecraft().player) {
            if (BlockHitting.Instance.isEnable()) {
                ...
                if (Minecraft.getMinecraft().player.isUsingItem()) {
                    swingSpeed = BlockHitting.blockSwingSpeed.getValue();
                } else {
                    swingSpeed = BlockHitting.swingSpeed.getValue();
                }

                return 6 + (int)(6 - 6 * swingSpeed) * 2;
            }
        }
