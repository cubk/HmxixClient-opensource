...
import io.space.mod.move.NoSlow;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        EventPreUpdate eventPreUpdate = new EventPreUpdate(posX,getEntityBoundingBox().minY,posZ,rotationYaw,rotationPitch,onGround,isSprinting(),isSneaking());
        ...
        boolean flag = eventPreUpdate.isSprinting();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(eventPreUpdate.getX(), eventPreUpdate.getY(), eventPreUpdate.getZ(), eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
                }
                else if (flag2)
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(eventPreUpdate.getX(), eventPreUpdate.getY(), eventPreUpdate.getZ(), eventPreUpdate.isOnGround()));
                }
                else if (flag3)
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer.C05PacketPlayerLook(eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
                }
                else
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer(eventPreUpdate.isOnGround()));
                }
            }
            else
            {
                this.sendQueue.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, eventPreUpdate.getYaw(), eventPreUpdate.getPitch(), eventPreUpdate.isOnGround()));
                flag2 = false;
            }
    ...
    {
        boolean flag = CommandManager.Instance.onChat(message);
        if (flag) return;
    ...
    public void onLivingUpdate()
    {
        ...
        boolean noSlowEnable = NoSlow.Instance.isEnable();
        boolean cantNoSlow = !noSlowEnable;

        if (noSlowEnable) {
            if (NoSlow.onlyIncludesSword()) {
                if (getHeldItem() != null) {
                    if (getHeldItem().getItem() instanceof ItemFood) {
                        cantNoSlow = true;
                    } else if (getHeldItem().getItem() instanceof ItemBow) {
                        cantNoSlow = true;
                    } else if (getHeldItem().getItem() instanceof ItemPotion && !ItemPotion.isSplash(mc.player.getHeldItem().getMetadata())) {
                        cantNoSlow = true;
                    }
                }
            }
        ...
        if (this.isUsingItem() && !this.isRiding() && !cantNoSlow && !NoSlow.multiplier.getValue().equals(1.0)) {
            this.movementInput.moveStrafe *= NoSlow.multiplier.getValue().floatValue();
            this.movementInput.moveForward *= NoSlow.multiplier.getValue().floatValue();
        }
    ...
    public boolean isForwardMoving() {
