...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn)
    {
        ...
        if (GlobalSetting.Instance.getFakeForge().getValue()) {
            this.netManager.sendPacket(new C17PacketCustomPayload("MC|Brand", (new PacketBuffer(Unpooled.buffer())).writeString("fml,forge")));
        } else this.netManager.sendPacket(new C17PacketCustomPayload("MC|Brand", (new PacketBuffer(Unpooled.buffer())).writeString(ClientBrandRetriever.getClientModName())));
    }
    ...
    public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn)
    {
        ...
        final NetworkPlayerInfo playerInfo = this.getPlayerInfo(packetIn.getPlayer());

        if (playerInfo == null) {
            return;
        }

        final GameProfile gameProfile = playerInfo.getGameProfile();
        EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(this.gameController.world, gameProfile);
