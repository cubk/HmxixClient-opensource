...
import io.space.mod.visual.AntiOpaqueBlock;
import io.space.mod.visual.BlockHitting;
import io.space.mod.visual.FireRenderer;
...
public class ItemRenderer
{
    ...
    {
        final boolean smoothSwingEnable = BlockHitting.Instance.isEnable() && BlockHitting.smoothHit.getValue();
        ...
        if (smoothSwingEnable) {
            GlStateManager.translate(j0 * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI) + BlockHitting.swingX.getValue(),
                    j1 * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI * 2.0F) + BlockHitting.swingY.getValue(),
                    j2 * MathHelper.sin(swingProgress * (float) Math.PI) + BlockHitting.swingZ.getValue());
        } else {
            float f = j0 * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
            float f1 = j1 * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI * 2.0F);
            float f2 = j2 * MathHelper.sin(swingProgress * (float) Math.PI);
            GlStateManager.translate(f, f1, f2);
        }
    }
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0)
                {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            ...
                            doSwordBlock(f,swingProgress,partialTicks);
                            break;
    ...
    private void doSwordBlock(float equippedProgress,float swingProgress,float partialTicks) {
        if (BlockHitting.Instance.isEnable()) {
            final float exhibition_sin = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
            ...
            switch (BlockHitting.mode.getValue()) {
                case "Exhibition":
                    transformFirstPersonItem(equippedProgress / 2.0f, 0.0f);
                    GL11.glRotatef(-exhibition_sin * 40.0f / 2.0f, exhibition_sin / 2.0f, -0.0f, 9.0f);
                    GL11.glRotatef(-exhibition_sin * 30.0f, 1.0f, exhibition_sin / 2.0f, -0.0f);
                ...
                case "None":
                    ...
                    break;
                case "HaoGe":
                    transformFirstPersonItem(equippedProgress, 0f);
                    doBlockTransformations();
                    final float sin = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.1415927F);
                    GlStateManager.rotate(-sin * 80.0F / 2.0F, 0f, sin, sin * 0.85f);
                    GlStateManager.rotate(-sin * 80.0F, 1, -0.2F, -0.0F);
                    break;
                case "Slowly":
                    this.transformFirstPersonItem(-0.15f,swingProgress);
                    doBlockTransformations();
                    break;
                case "Vanilla":
                default:
    ...
    public void renderOverlays(float partialTicks)
    {
        ...
        if (this.mc.player.isEntityInsideOpaqueBlock() && !AntiOpaqueBlock.Instance.isEnable())
        {
    ...
    {
        if (FireRenderer.Instance.isEnable() && FireRenderer.mode.isCurrentMode("None")) {
            return;
        }
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            float f6 = f5 + f;
            float f7 = 0.0F - f / 2.0F - ((FireRenderer.Instance.isEnable() && FireRenderer.mode.isCurrentMode("Low")) ? FireRenderer.lowY.getValue().floatValue() : 0.0F);
            float f8 = f7 + f;
            float f9 = -0.5F;
