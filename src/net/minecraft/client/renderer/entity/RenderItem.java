...
import io.space.mod.visual.EnchantColor;
    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                if (EnchantColor.Instance.isEnable()) {
                    if (EnchantColor.rainbow.getValue()) {
                        enchantColor = RenderUtils.rainbow(3500f,1f,1f,1);
                    } else {
                        enchantColor = EnchantColor.colorValue.getValue();
                    }
                }

                this.renderModel(model, enchantColor);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
                GlStateManager.scale(8.0F, 8.0F, 8.0F);
