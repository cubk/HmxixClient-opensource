...
import io.space.mod.visual.BetterGui;
import io.space.mod.visual.HUD;
import io.space.mod.visual.NoPumpkinHead;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        模糊渲染程序.onRenderGameOverlay(this.mc.getFramebuffer(), scaledresolution);
        EventManager.call(new Event2D(scaledresolution,partialTicks));
    ...
    private double tooltipAnimationX = new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() / 2.0 - 91;
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0F;
            if (HUD.Instance.isEnable() && HUD.betterTooltip.getValue()) {
                RenderUtils.drawRect(i - 91, sr.getScaledHeight_double() - 22,i + 91,sr.getScaledHeight_double(),RenderUtils.getRGB(0,0,0,50));
                final int j = i - 91 + entityplayer.inventory.currentItem * 20;
                tooltipAnimationX = RenderUtils.getAnimationStateEasing(tooltipAnimationX,j,20);
                RenderUtils.drawRect(tooltipAnimationX, sr.getScaledHeight_double() - 22, tooltipAnimationX + 22, sr.getScaledHeight_double(),RenderUtils.getRGB(0,0,0,100));
            } else {
    ...
    {
        final int FONT_HEIGHT = (BetterGui.Instance.isEnable() && BetterGui.scoreboardFont.getValue()) ? FontManager.default16.getHeight() + 2 : getFontRenderer().FONT_HEIGHT;
        final boolean enableBetterGui = BetterGui.Instance.isEnable() && BetterGui.scoreboardFont.getValue();
        ...
        int i = enableBetterGui ? FontManager.default16.getStringWidth(objective.getDisplayName()) : this.getFontRenderer().getStringWidth(objective.getDisplayName());

        for (Score score : collection)
        {
            ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score.getPlayerName());
            String s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()) + ": " + EnumChatFormatting.RED + score.getScorePoints();
            i = Math.max(i, enableBetterGui ? FontManager.default16.getStringWidth(s) : this.getFontRenderer().getStringWidth(s));
        }
        ...
        for (Score score1 : collection)
        {
            ...
            if (BetterGui.Instance.isEnable()) {
                k += BetterGui.scoreboardY.getValue();
            }
            ...
            if (enableBetterGui) {
                FontManager.default16.drawString(s1,l1,k,553648127);

                if (BetterGui.scoreboardPoint.getValue()) FontManager.default16.drawString(s2,l - FontManager.default16.getStringWidth(s2) + 8,k,553648127);
            } else {
                this.getFontRenderer().drawString(s1, l1, k, 553648127);

                if (BetterGui.Instance.isEnable()) {
                    if (BetterGui.scoreboardPoint.getValue()) this.getFontRenderer().drawString(s2, l - this.getFontRenderer().getStringWidth(s2), k, 553648127);
                } else this.getFontRenderer().drawString(s2, l - this.getFontRenderer().getStringWidth(s2), k, 553648127);
            }
            ...
            if (j == collection.size())
            {
                ...
                if (enableBetterGui) {
                    FontManager.default16.drawString(s3, l1 + i / 2.0 - FontManager.default16.getStringWidth(s3) / 2.0, k - FONT_HEIGHT, 553648127);
                } else this.getFontRenderer().drawString(s3, l1 + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, k - FONT_HEIGHT, 553648127);
            }
        }
    }
    ...
    {
        if (NoPumpkinHead.Instance.isEnable()) return;
