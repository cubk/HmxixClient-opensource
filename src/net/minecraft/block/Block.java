...
import io.space.mod.world.XRay;
...
public class Block
{
    ...
    {
        if (XRay.Instance.isEnable()) {
            for (int i : XRay.antiXRayBlocks) {
                if (this == getBlockById(i)) {
                    return true;
                }
            }
        }
    ...
    public int getMixedBrightnessForBlock(IBlockAccess worldIn, BlockPos pos)
    {
        ...
        int i = worldIn.getCombinedLight(pos, XRay.Instance.isEnable() ? 100000 : block.getLightValue());
    ...
    {
        EventBlockRenderSide event = new EventBlockRenderSide(this,worldIn, pos, side, maxX, maxY, maxZ, minX, minY, minZ);
        EventManager.call(event);

        if (XRay.Instance.isEnable()) {
            for (int i : XRay.antiXRayBlocks) {
                if (this == getBlockById(i)) {
                    return event.needToRender();
                }
            }
        }
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
        ...
        if (collidingEntity == Minecraft.getMinecraft().player) {
            final EventCollideWithBlock e = new EventCollideWithBlock(this,pos,axisalignedbb,state,list);
            EventManager.call(e);
